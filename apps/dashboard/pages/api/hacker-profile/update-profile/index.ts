...
const handler: NextApiHandler = async (req, res) => {
  if (req.method == 'PUT') {
    ...
    const resCheckIfExist = await nextnexus
      .getClient()
      .from('user_profiles')
      .select('user_id')
      .eq('user_id', user);
    ...
    const resCheckParticipantExist = await nextnexus
      .getClient()
      .from('participants')
      .select('id')
      .eq('id', user);
    ...
    if (resCheckParticipantExist.error) {
      resUpdateParticipants = await nextnexus
        .getClient()
        .from('participants')
        .insert({
          id: user,
    ...
    const resUpdateUserProfiles = await nextnexus
      .getClient()
      .from('user_profiles')
      .update({
